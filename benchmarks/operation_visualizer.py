...
class BenchmarkVisualizer:
	...
	def plot_memory_comparison(
		...
		ax1.set_xticks(x)
		ax1.set_xticklabels(operations, rotation=0, ha='right')
		ax1.legend()
		...
		ax2.set_xticks(x)
		ax2.set_xticklabels(operations, rotation=0, ha='right')
		ax2.legend()
	...
	def plot_speed_comparison(
		...
		# 바 그래프 생성
		ax.bar(x - width/2, naive_times, width, label='Naive', color='#ff7f7f', alpha=0.8)
		ax.bar(x + width/2, optimized_times, width, label='Optimized', color='#7fbf7f', alpha=0.8)
		...
		# 개선율만 간단히 표시
		for i, (naive, opt) in enumerate(zip(naive_times, optimized_times)):
			if naive > 0 and opt > 0:
				improvement = ((naive - opt) / naive) * 100
				ax.text(i, max(naive, opt) * 1.1, f'{improvement:.1f}%', 
					ha='center', va='bottom', fontweight='bold', color='green')
	...
	def plot_model_comparison(
		...
		ax2.set_xticks(x)
		ax2.set_xticklabels(models, rotation=0, ha='right')
		ax2.grid(True, alpha=0.3)
